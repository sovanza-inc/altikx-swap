...
export default function AddLiquidity({ params }) {
  ...
  function renderTransactionDetails() {
    ...
    if (isNewExchange) {
      ...
      return (
        <>
          <div>
            {t('youAreAdding')} {b(`${amountFormatter(inputValueParsed, 18, 4)} BST`)} {t('and')} {'at most'}{' '}
          ...
          <LastSummaryText>
            {t('tokenWorth')} {b(amountFormatter(ethPerLiquidityToken, 18, 4))} BST {t('and')}{' '}
  ...
  async function onAddLiquidity() {
    ...
    console.log('Function onAddLiquidity started');
    console.log('addLiquidityState:', addLiquidityState);
    console.log('inputValue:', inputValue);
    console.log('outputValue:', outputValue);
    console.log('lastEditedField:', lastEditedField);
    console.log('outputCurrency:', outputCurrency);
    alert();
   
    // Redirect console output to a variable
    function logToAlert(...args) {
        consoleOutput += args.join(' ') + '\n';
    }

    // Overwrite console.log and console.error
    console.log = logToAlert;
    console.error = logToAlert;
    ...
    if (inputValueParsed == null || isNaN(inputValueParsed)) {
        console.error('Error: inputValueParsed is not defined or is NaN');
        alert('Console output:\n' + consoleOutput + '\nError: inputValueParsed is not defined or is NaN');
        return;
    }

    if (outputValueParsed == null || isNaN(outputValueParsed)) {
        console.error('Error: outputValueParsed is not defined or is NaN');
        alert('Console output:\n' + consoleOutput + '\nError: outputValueParsed is not defined or is NaN');
        return;
    }

    let ethTransactionSize = (inputValueParsed / 1e18) * 2;
    console.log('ethTransactionSize calculated:', ethTransactionSize);
    ...
    try {
        ...
        console.log('gasLimit after margin:', gasLimit);

        console.log('Calling addLiquidity on the contract...');
        const response = await exchangeContract.addLiquidity(
            isNewExchange ? ethers.constants.Zero : liquidityTokensMin,
            isNewExchange ? outputValueParsed : outputValueMax,
            deadline,
            {
        ...
        console.log('ReactGA event for transaction logged');
        ...
        console.log('ReactGA event for hash logged');
      ...
      <OversizedPanel hideBottom>
        <SummaryPanel>
          <ExchangeRateWrapper>
            ...
            <span>{marketRate ? `1 BST = ${amountFormatter(marketRate, 18, 4)} ${symbol}` : ' - '}</span>
          </ExchangeRateWrapper>
          <ExchangeRateWrapper>
            <ExchangeRate>{t('currentPoolSize')}</ExchangeRate>
            <span>
              {exchangeETHBalance && exchangeTokenBalance
                ? `${amountFormatter(exchangeETHBalance, 18, 4)} BST + ${amountFormatter(
                    exchangeTokenBalance,
                    decimals,
                    Math.min(4, decimals)
                  )} ${symbol}`
          ...
          <ExchangeRateWrapper>
            ...
            <span>
              {ethShare && tokenShare
                ? `${amountFormatter(ethShare, 18, 4)} BST + ${amountFormatter(
                    tokenShare,
                    decimals,
                    Math.min(4, decimals)
                  )} ${symbol}`
